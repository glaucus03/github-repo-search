...
test.describe("検索機能テスト", () => {
  ...
  test("ページが正しく読み込まれる", async ({ page }) => {
    ...
    // メインヘッダーの確認
    await expect(
      page.getByRole("heading", { name: "GitHub Repository Search" }),
    ).toBeVisible();
  ...
  test("検索機能が正常に動作する", async ({ page }) => {
    ...
    await expect(page.getByText(/「react」の検索結果は/)).toBeVisible({
      timeout: 10000,
    });
  ...
  test("検索結果のリポジトリカードに必要な情報が表示される", async ({
    page,
  }) => {
    // 検索を実行
  ...
  test("タイトルクリックで初期状態にリセットされる", async ({ page }) => {
    ...
    // タイトルをクリック
    await page
      .getByRole("heading", { name: "GitHub Repository Search" })
      .click();
