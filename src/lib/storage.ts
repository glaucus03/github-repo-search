...
export class FavoritesStorage {
  ...
  static addToFavorites(repository: GitHubRepository): boolean {
    ...
    // 既に存在するかチェック
    const existingIndex = favorites.findIndex(repo => repo.id === repository.id)
    if (existingIndex !== -1) {
      return false // 既に存在する
    }
  ...
  static removeFromFavorites(repositoryId: number): boolean {
    const favorites = this.getFavorites()
    const filteredFavorites = favorites.filter(repo => repo.id !== repositoryId)
    return LocalStorageManager.setItem(STORAGE_KEYS.FAVORITES, filteredFavorites)
  }
...
export class UISettingsStorage {
  ...
  static getSettings(): UISettings {
    return LocalStorageManager.getItem(STORAGE_KEYS.UI_SETTINGS, this.DEFAULT_SETTINGS)
  }
  ...
  static resetSettings(): boolean {
    return LocalStorageManager.setItem(STORAGE_KEYS.UI_SETTINGS, this.DEFAULT_SETTINGS)
  }
}
  ...
  static importData(jsonData: string): boolean {
    try {
      ...
      if (data.searchHistory && Array.isArray(data.searchHistory)) {
        LocalStorageManager.setItem(STORAGE_KEYS.SEARCH_HISTORY, data.searchHistory)
      }
