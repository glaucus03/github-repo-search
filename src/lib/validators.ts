...
export function validateSearchQuery(query: string): {
  ...
  if (trimmedQuery.length < SEARCH.MIN_QUERY_LENGTH) {
    errors.push(
      `検索クエリは${SEARCH.MIN_QUERY_LENGTH}文字以上で入力してください`,
    );
  }

  if (trimmedQuery.length > SEARCH.MAX_QUERY_LENGTH) {
    errors.push(
      `検索クエリは${SEARCH.MAX_QUERY_LENGTH}文字以下で入力してください`,
    );
  }
...
export function validateGitHubRepositoryUrl(url: string): {
  ...
  if (!match) {
    return {
      isValid: false,
      error: "有効なGitHubリポジトリURLを入力してください",
    };
  }
...
export function validateGitHubUserUrl(url: string): {
  ...
  if (!match) {
    return {
      isValid: false,
      error: "有効なGitHubユーザーURLを入力してください",
    };
  }
...
export function validatePageNumber(page: number | string): {
  ...
  if (pageNum > 34) {
    // GitHub APIの制限（1000件 ÷ 30件/ページ ≈ 34ページ）
