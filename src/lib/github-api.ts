...
class GitHubApiClient {
  ...
  // リポジトリ検索（再試行機能付き）
  async searchRepositories(query: GitHubSearchQuery): Promise<GitHubSearchResponse> {
    ...
    if (query.per_page) searchParams.append('per_page', query.per_page.toString())
    if (query.page) searchParams.append('page', query.page.toString())

    return retryWithBackoff(
      () => this.request<GitHubSearchResponse>(`/search/repositories?${searchParams.toString()}`),
      2, // 最大2回再試行
      1000 // 1秒のベースディレイ
    )
  }
  ...
  async getRepositoryContributors(owner: string, repo: string): Promise<GitHubContributor[]> {
    return this.request<GitHubContributor[]>(`/repos/${owner}/${repo}/contributors`)
  }

  // リポジトリのREADMEを取得
  async getRepositoryReadme(owner: string, repo: string): Promise<GitHubReadmeResponse> {
    return this.request<GitHubReadmeResponse>(`/repos/${owner}/${repo}/readme`)
  }

  // レート制限情報を取得
  async getRateLimit(): Promise<GitHubRateLimit> {
    const response = await this.request<{ rate: GitHubRateLimit }>('/rate_limit')
    return response.rate
  }

  // 人気のリポジトリを取得（検索クエリを使用）
  async getPopularRepositories(language?: string, limit = 30): Promise<GitHubRepository[]> {
    let query = 'stars:>1'
  ...
  // 最近更新されたリポジトリを取得
  async getRecentlyUpdatedRepositories(language?: string, limit = 30): Promise<GitHubRepository[]> {
    let query = 'stars:>10'
