...
// GitHubRepository型は使用されていないがインポートは残す（将来使用される可能性があるため）
 ...
 * リポジトリの品質スコア計算用の型
 */
export interface RepositoryQualityProps {
 ...
 * リポジトリの品質スコア計算
 */
export function calculateRepositoryQuality(
  repo: RepositoryQualityProps,
): number {
  let score = 0;
 ...
 * 検索統計計算用の最小型
 */
export type RepositoryForStats = RepositoryQualityProps;

/**
 * 検索結果の統計情報を計算
 */
export function calculateSearchStatistics(repositories: RepositoryForStats[]): {
  totalCount: number;
  averageStars: number;
  averageForks: number;
  ...
  repositories.forEach((repo) => {
    ...
    languageDistribution.set(
      language,
      (languageDistribution.get(language) || 0) + 1,
    );
  });
