...
jest.mock('../constants', () => ({
  REGEX_PATTERNS: {
    GITHUB_REPO_URL: /^https:\/\/github\.com\/([a-zA-Z0-9\-._]+)\/([a-zA-Z0-9\-._]+)\/?$/,
...
describe('バリデーション関数', () => {
  describe('validateSearchQuery', () => {
    ...
    it('nullやundefinedを拒否する', () => {
      const result1 = validateSearchQuery(null as any)
      const result2 = validateSearchQuery(undefined as any)
    ...
    it('長すぎるクエリを拒否する', () => {
      ...
      expect(result.isValid).toBe(false)
      expect(result.errors).toContain('検索クエリは256文字以下で入力してください')
    })
  ...
  describe('validateGitHubRepositoryUrl', () => {
    it('有効なGitHubリポジトリURLを受け入れる', () => {
      const result = validateGitHubRepositoryUrl('https://github.com/facebook/react')
      expect(result.isValid).toBe(true)
    ...
    it('末尾スラッシュ付きのURLを受け入れる', () => {
      const result = validateGitHubRepositoryUrl('https://github.com/microsoft/vscode/')
      expect(result.isValid).toBe(true)
    ...
    it('無効なURLを拒否する', () => {
      const result = validateGitHubRepositoryUrl('https://gitlab.com/user/repo')
      expect(result.isValid).toBe(false)
    ...
    it('特殊文字を含むユーザー名/リポジトリ名を受け入れる', () => {
      const result = validateGitHubRepositoryUrl('https://github.com/user-name_123/repo.name-test')
      expect(result.isValid).toBe(true)
    })
  })
  ...
  describe('validateFileName', () => {
    ...
    it('危険な文字を含むファイル名を拒否する', () => {
      const dangerousNames = ['file<test.txt', 'file>test.txt', 'file:test.txt']
...
describe('FormValidator', () => {
  ...
  describe('validate', () => {
    it('複数のルールでバリデーションを実行する', () => {
      ...
      expect(validator.getFieldErrors('username')).toContain('5文字以上で入力してください')
    })

    it('すべてのルールが通る場合は有効とする', () => {
      validator.validate('username', 'testuser', [required, minLength(5), maxLength(20)])
