...
import type { GitHubSearchResponse, GitHubRepository, GitHubRateLimit } from '@/types/github'
import { createMockGitHubUser, createMockGitHubRepository, createMockGitHubRateLimit, createMockResponse, createMockErrorResponse } from '../../test-utils'
  ...
  describe('searchRepositories', () => {
    it('検索クエリを正しく実行する', async () => {
      const mockResponse: GitHubSearchResponse = {
        ...
        items: [createMockGitHubRepository({
          name: 'test-repo',
...
describe('ヘルパー関数', () => {
  describe('createSearchQuery', () => {
    ...
    it('複数のオプションを組み合わせる', () => {
      ...
      expect(result).toBe('gatsby language:javascript stars:>=100 created:>2022-01-01')
    })
  })
