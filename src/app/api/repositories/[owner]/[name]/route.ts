...
export async function GET(
  ...
  try {
    ...
    const includeContributors = searchParams.get('include_contributors') === 'true'
    ...
    if (includeLanguages) {
      additionalDataPromises.push(githubApi.getRepositoryLanguages(owner, name))
      additionalDataKeys.push('languages')
    }

    if (includeContributors) {
      additionalDataPromises.push(githubApi.getRepositoryContributors(owner, name))
