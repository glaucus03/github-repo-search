...
describe('searchStore', () => {
  ...
  it('結果の追加が正しく動作する', () => {
    const { result } = renderHook(() => useSearchStore())
    const initialResults = [{ id: 1, name: 'repo1' }] as any
    const additionalResults = [{ id: 2, name: 'repo2' }] as any
    ...
    expect(result.current.results).toEqual([...initialResults, ...additionalResults])
  })
  ...
  it('検索リセットが正しく動作する', () => {
    ...
    act(() => {
      result.current.setQuery('react')
      result.current.setResults([{ id: 1, name: 'test' }] as any)
      result.current.setLoading(true)
  ...
  it('リポジトリ品質計算が正しく動作する', () => {
    const { result } = renderHook(() => useSearchStore())
    const mockRepo = { stargazers_count: 100 } as any
