      ...
      (set, _get) => ({
        ...
        toggleOrder: () =>
          set(
            (state) => ({
              selectedOrderOption:
                state.selectedOrderOption === "desc" ? "asc" : "desc",
            }),
            false,
            "ui/toggleOrder",
          ),
        ...
        removeFromFavorites: (repositoryId) =>
          set(
            (state) => {
              const success =
                FavoritesStorage.removeFromFavorites(repositoryId);
              if (success) {
...
export const selectSearchFormState = (state: UIStore) =>
  state.isSearchFormExpanded;
