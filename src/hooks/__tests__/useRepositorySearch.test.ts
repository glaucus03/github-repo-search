import { renderHook, act } from "@testing-library/react";

import { useRepositorySearch } from "../useRepositorySearch";

// Mock SWR
jest.mock("swr", () => ({
  __esModule: true,
  default: jest.fn(() => ({
    data: null,
    error: null,
    isLoading: false,
    mutate: jest.fn(),
  })),
}));

// Mock search domain functions
jest.mock("@/lib/search-domain", () => ({
  validateSearchQuery: jest.fn(() => ({ isValid: true, errors: [] })),
  buildGitHubSearchQuery: jest.fn((params) => params.query),
  createPopularRepositoryQuery: jest.fn(() => "stars:>1000"),
  createRecentRepositoryQuery: jest.fn(() => "pushed:>2023-01-01"),
}));

// Mock stores
jest.mock("@/store/searchStore");
jest.mock("@/store/uiStore");

describe("useRepositorySearch", () => {
  beforeEach(() => {
    jest.clearAllMocks();

    // モックストアの設定
    const mockSearchStore = jest.requireMock("@/store/searchStore");
    mockSearchStore.useSearchStore.mockReturnValue({
      query: "react",
      results: [],
      loading: false,
      error: null,
      hasMore: true,
      totalCount: 0,
      page: 1,
      setQuery: jest.fn(),
      setResults: jest.fn(),
      addResults: jest.fn(),
      setLoading: jest.fn(),
      setError: jest.fn(),
      setTotalCount: jest.fn(),
      setHasMore: jest.fn(),
      incrementPage: jest.fn(),
      resetResults: jest.fn(),
      addToHistory: jest.fn(),
    });

    mockSearchStore.useSearchStore.getState = jest.fn(() => ({
      query: "react",
      page: 1,
      searchOptions: {
        sort: "best-match",
        order: "desc",
      },
      setQuery: jest.fn(),
    }));

    mockSearchStore.buildSearchQuery = jest.fn(() => ({
      q: "react",
      per_page: 30,
      page: 1,
    }));

    const mockUIStore = jest.requireMock("@/store/uiStore");
    mockUIStore.useUIStore.mockReturnValue({
      addNotification: jest.fn(),
    });
  });

  it("初期状態が正しく設定される", () => {
    const { result } = renderHook(() => useRepositorySearch());

    expect(result.current.results).toEqual([]);
    expect(result.current.loading).toBe(false);
    expect(result.current.error).toBe(null);
    expect(result.current.hasMore).toBe(true);
    expect(result.current.totalCount).toBe(0);
  });

  it("search関数が正しく動作する", async () => {
    const mockSearchStore = jest.requireMock("@/store/searchStore");
    const mockSetQuery = jest.fn();
    const mockResetResults = jest.fn();
    const mockAddToHistory = jest.fn();

    mockSearchStore.useSearchStore.getState.mockReturnValue({
      query: "react",
      page: 1,
      searchOptions: {
        sort: "best-match",
        order: "desc",
      },
      setQuery: mockSetQuery,
    });

    mockSearchStore.useSearchStore.mockReturnValue({
      query: "react",
      results: [],
      loading: false,
      error: null,
      hasMore: true,
      totalCount: 0,
      page: 1,
      setQuery: mockSetQuery,
      setResults: jest.fn(),
      addResults: jest.fn(),
      setLoading: jest.fn(),
      setError: jest.fn(),
      setTotalCount: jest.fn(),
      setHasMore: jest.fn(),
      incrementPage: jest.fn(),
      resetResults: mockResetResults,
      addToHistory: mockAddToHistory,
    });

    const { result } = renderHook(() => useRepositorySearch());

    await act(async () => {
      await result.current.search("react");
    });

    expect(mockSetQuery).toHaveBeenCalledWith("react");
    expect(mockResetResults).toHaveBeenCalled();
    expect(mockAddToHistory).toHaveBeenCalledWith("react");
  });

  it("バリデーションエラーの処理が正しく動作する", async () => {
    const mockSearchDomain = jest.requireMock("@/lib/search-domain");
    const mockSearchStore = jest.requireMock("@/store/searchStore");
    const mockSetError = jest.fn();

    mockSearchDomain.validateSearchQuery.mockReturnValueOnce({
      isValid: false,
      errors: ["クエリが無効です"],
    });

    mockSearchStore.useSearchStore.mockReturnValue({
      query: "",
      results: [],
      loading: false,
      error: null,
      hasMore: true,
      totalCount: 0,
      page: 1,
      setQuery: jest.fn(),
      setResults: jest.fn(),
      addResults: jest.fn(),
      setLoading: jest.fn(),
      setError: mockSetError,
      setTotalCount: jest.fn(),
      setHasMore: jest.fn(),
      incrementPage: jest.fn(),
      resetResults: jest.fn(),
      addToHistory: jest.fn(),
    });

    const { result } = renderHook(() => useRepositorySearch());

    await act(async () => {
      await result.current.search("");
    });

    expect(mockSetError).toHaveBeenCalledWith("クエリが無効です");
  });

  it("loadMore関数が正しく動作する", async () => {
    const mockSearchStore = jest.requireMock("@/store/searchStore");
    const mockIncrementPage = jest.fn();

    mockSearchStore.useSearchStore.mockReturnValue({
      query: "react",
      results: [],
      loading: false,
      error: null,
      hasMore: true,
      totalCount: 100,
      page: 1,
      setQuery: jest.fn(),
      setResults: jest.fn(),
      addResults: jest.fn(),
      setLoading: jest.fn(),
      setError: jest.fn(),
      setTotalCount: jest.fn(),
      setHasMore: jest.fn(),
      incrementPage: mockIncrementPage,
      resetResults: jest.fn(),
      addToHistory: jest.fn(),
    });

    const { result } = renderHook(() => useRepositorySearch());

    await act(async () => {
      await result.current.loadMore();
    });

    expect(mockIncrementPage).toHaveBeenCalled();
  });

  it("オプションでenabled=falseの場合、自動実行されない", () => {
    const { result } = renderHook(() =>
      useRepositorySearch({ enabled: false }),
    );

    expect(result.current.results).toEqual([]);
  });

  it("hasMoreがfalseの場合、loadMoreが実行されない", async () => {
    const mockSearchStore = jest.requireMock("@/store/searchStore");
    const mockIncrementPage = jest.fn();

    mockSearchStore.useSearchStore.mockReturnValue({
      query: "react",
      results: [],
      loading: false,
      error: null,
      hasMore: false, // hasMoreがfalse
      totalCount: 30,
      page: 1,
      setQuery: jest.fn(),
      setResults: jest.fn(),
      addResults: jest.fn(),
      setLoading: jest.fn(),
      setError: jest.fn(),
      setTotalCount: jest.fn(),
      setHasMore: jest.fn(),
      incrementPage: mockIncrementPage,
      resetResults: jest.fn(),
      addToHistory: jest.fn(),
    });

    const { result } = renderHook(() => useRepositorySearch());

    await act(async () => {
      await result.current.loadMore();
    });

    // hasMoreがfalseなのでincrementPageは呼ばれない
    expect(mockIncrementPage).not.toHaveBeenCalled();
  });

  it("ローディング中はloadMoreが実行されない", async () => {
    const mockSearchStore = jest.requireMock("@/store/searchStore");
    const mockIncrementPage = jest.fn();

    mockSearchStore.useSearchStore.mockReturnValue({
      query: "react",
      results: [],
      loading: true, // ローディング中
      error: null,
      hasMore: true,
      totalCount: 100,
      page: 1,
      setQuery: jest.fn(),
      setResults: jest.fn(),
      addResults: jest.fn(),
      setLoading: jest.fn(),
      setError: jest.fn(),
      setTotalCount: jest.fn(),
      setHasMore: jest.fn(),
      incrementPage: mockIncrementPage,
      resetResults: jest.fn(),
      addToHistory: jest.fn(),
    });

    const { result } = renderHook(() => useRepositorySearch());

    await act(async () => {
      await result.current.loadMore();
    });

    // ローディング中なのでincrementPageは呼ばれない
    expect(mockIncrementPage).not.toHaveBeenCalled();
  });

  it("空のクエリでsearchが呼ばれた場合、履歴に追加されない", async () => {
    const mockSearchDomain = jest.requireMock("@/lib/search-domain");
    const mockSearchStore = jest.requireMock("@/store/searchStore");
    const mockAddToHistory = jest.fn();

    mockSearchDomain.validateSearchQuery.mockReturnValueOnce({
      isValid: false,
      errors: ["クエリが空です"],
    });

    mockSearchStore.useSearchStore.mockReturnValue({
      query: "",
      results: [],
      loading: false,
      error: null,
      hasMore: true,
      totalCount: 0,
      page: 1,
      setQuery: jest.fn(),
      setResults: jest.fn(),
      addResults: jest.fn(),
      setLoading: jest.fn(),
      setError: jest.fn(),
      setTotalCount: jest.fn(),
      setHasMore: jest.fn(),
      incrementPage: jest.fn(),
      resetResults: jest.fn(),
      addToHistory: mockAddToHistory,
    });

    const { result } = renderHook(() => useRepositorySearch());

    await act(async () => {
      await result.current.search(""); // 空のクエリ
    });

    // 空のクエリは履歴に追加されない
    expect(mockAddToHistory).not.toHaveBeenCalled();
  });

  it("デバウンスされたクエリの変更が処理される", async () => {
    const mockSearchStore = jest.requireMock("@/store/searchStore");
    const mockSetQuery = jest.fn();

    mockSearchStore.useSearchStore.mockReturnValue({
      query: "react",
      results: [],
      loading: false,
      error: null,
      hasMore: true,
      totalCount: 0,
      page: 1,
      setQuery: mockSetQuery,
      setResults: jest.fn(),
      addResults: jest.fn(),
      setLoading: jest.fn(),
      setError: jest.fn(),
      setTotalCount: jest.fn(),
      setHasMore: jest.fn(),
      incrementPage: jest.fn(),
      resetResults: jest.fn(),
      addToHistory: jest.fn(),
    });

    const { result } = renderHook(() => useRepositorySearch());

    // search関数の存在を確認
    expect(typeof result.current.search).toBe("function");
    expect(typeof result.current.loadMore).toBe("function");
  });
});
