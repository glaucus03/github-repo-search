...
import { validateSearchQuery as validateQuery } from '@/lib/validators'
import { 
  validateSearchQuery, 
...
export function SearchForm({ onSearch, className }: SearchFormProps) {
  ...
  // 拡張フィルタ用の状態（storeと連携）
  const [selectedLanguage, setSelectedLanguage] = useState<string | null>(searchOptions.language || null)
  const [minStars, setMinStars] = useState<number | null>(searchOptions.minStars || null)
  const [maxStars, setMaxStars] = useState<number | null>(searchOptions.maxStars || null)
  ...
  }, [selectedLanguage, minStars, maxStars, selectedSortOption, selectedOrderOption])

  // クイック検索（人気のリポジトリ）
  const handlePopularSearch = () => {
    const popularQuery = createPopularRepositoryQuery(selectedLanguage || undefined)
            ...
            <div className="absolute top-full left-0 right-0 z-10 mt-2 bg-gray-800 border border-gray-600 rounded-lg shadow-lg max-h-60 overflow-y-auto">
              <div className="p-2">
                ...
                {searchHistory.length > 0 && (
                  <>
                    <div className="text-xs text-gray-400 mb-2 px-2">検索履歴</div>
                ...
                {suggestions.length > 0 && (
                  <>
                    <div className="text-xs text-gray-400 mb-2 px-2 mt-2">おすすめ検索</div>
      ...
      {showAdvancedFilters && (
        <div className="bg-gray-800 border border-gray-600 rounded-lg p-6 mb-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
            {/* 言語選択 */}
            <div>
              <label className="block text-sm font-medium text-gray-300 mb-2">プログラミング言語</label>
              <select
            ...
            {/* ソート選択 */}
            <div>
              <label className="block text-sm font-medium text-gray-300 mb-2">ソート</label>
              <select
            ...
            {/* 順序選択 */}
            <div>
              <label className="block text-sm font-medium text-gray-300 mb-2">順序</label>
              <select
                value={selectedOrderOption}
                onChange={(e) => setOrderOption(e.target.value as 'asc' | 'desc')}
                className="w-full bg-gray-700 border border-gray-600 rounded-lg px-3 py-2 text-white focus:border-blue-500 focus:outline-none"
              >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            <div>
              <label className="block text-sm font-medium text-gray-300 mb-2">最小スター数</label>
              <input
                type="number"
                placeholder="0"
            ...
            <div>
              <label className="block text-sm font-medium text-gray-300 mb-2">最大スター数</label>
              <input
                type="number"
                placeholder="無制限"
