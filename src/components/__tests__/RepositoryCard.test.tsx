...
jest.mock("@/lib/utils", () => ({
  ...
  formatRelativeTime: (_date: Date) => "2時間前",
...
jest.mock("@heroui/react", () => ({
  Card: ({ children, className, isPressable, onPress }: MockCardProps) => (
    <div
  ...
  CardFooter: ({ children, className }: MockCardFooterProps) => (
    <div className={className as string} data-testid="card-footer">
      {children as React.ReactNode}
    </div>
  ),
  Avatar: ({ alt, className }: MockAvatarProps) => (
    <div
      data-testid="avatar"
      className={className as string}
  ...
  Chip: ({ children, className }: MockChipProps) => (
    <span className={className as string} data-testid="chip">
      {children as React.ReactNode}
    </span>
  ),
  Button: ({ children, onClick, ...props }: MockButtonProps) => {
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
    const { isIconOnly, as, variant, ...buttonProps } = props;
    return (
      <button
  ...
  Link: ({ children, href, ...props }: MockLinkProps) => (
    <a href={href as string} role="link" {...props}>
      {children as React.ReactNode}
    </a>
  ),
}));
...
describe("RepositoryCard", () => {
  ...
  it("リポジトリ情報を正しく表示する", () => {
    render(
      <RepositoryCard repository={mockRepository} onSelect={mockOnSelect} />,
    );
  ...
  it("お気に入り済みの場合は削除ボタンが表示される", () => {
    ...
    expect(mockUIStore.removeFromFavorites).toHaveBeenCalledWith(
      mockRepository.id,
    );
    expect(mockUIStore.addNotification).toHaveBeenCalledWith({
      type: "success",
  ...
  it("ライセンス情報が表示される", () => {
    ...
    render(
      <RepositoryCard repository={repoWithLicense} onSelect={mockOnSelect} />,
    );
  ...
  it("アーカイブ済みバッジが表示される", () => {
    ...
    render(
      <RepositoryCard repository={archivedRepo} onSelect={mockOnSelect} />,
    );
  ...
  it("テンプレートバッジが表示される", () => {
    ...
    render(
      <RepositoryCard repository={templateRepo} onSelect={mockOnSelect} />,
    );
