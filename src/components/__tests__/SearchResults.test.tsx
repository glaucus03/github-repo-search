...
const mockUseLiveSearchFn = jest.mocked(useLiveSearch);
...
jest.mock("@/components", () => ({
  Pagination: ({
    ...
    <div data-testid="pagination">
      <span>
        Page {currentPage as number} of {totalPages as number}
      </span>
      <button
        onClick={() =>
          (onPageChange as (page: number) => void)((currentPage as number) + 1)
        }
...
jest.mock("@heroui/react", () => ({
  Card: ({ children, className }: MockCardProps) => (
    <div className={className as string} data-testid="card">
      {children as React.ReactNode}
    </div>
  ),
  CardBody: ({ children, className }: MockCardBodyProps) => (
    <div className={className as string} data-testid="card-body">
      {children as React.ReactNode}
    </div>
  ),
  Spinner: ({ size }: MockSpinnerProps) => (
    <div data-testid="spinner" data-size={size as string}>
      Loading...
    </div>
  ),
}));
...
describe("SearchResults", () => {
  ...
  beforeEach(() => {
    jest.clearAllMocks();
    mockUseLiveSearchFn.mockReturnValue(mockUseLiveSearch);
  });
  ...
  describe("Loading states", () => {
    ...
    it("ライブサーチローディング状態を表示する", () => {
      mockUseLiveSearchFn.mockReturnValue({
        ...mockUseLiveSearch,
        loading: true,
      });

      render(<SearchResults {...defaultProps} searchQuery="react" />);
  ...
  describe("Error states", () => {
    it("エラー状態を表示する", () => {
      render(
        <SearchResults {...defaultProps} error="検索エラーが発生しました" />,
      );
  ...
  describe("No results states", () => {
    ...
    it("検索結果がない場合のメッセージを表示する", () => {
      render(<SearchResults {...defaultProps} searchQuery="nonexistent" />);
      ...
      expect(
        screen.getByText(
          "「nonexistent」に一致するリポジトリが見つかりませんでした",
        ),
      ).toBeInTheDocument();
    });
    ...
    it("ライブサーチで結果があるが実際の検索結果がない場合", () => {
      ...
      expect(
        screen.getByText("「react」の検索結果: 150件"),
      ).toBeInTheDocument();
      expect(
        screen.getByText("検索ボタンを押して結果を表示してください"),
      ).toBeInTheDocument();
    });
  });
  ...
  describe("Results display", () => {
    it("検索結果を正しく表示する", () => {
      ...
      expect(
        screen.getByText("Test repository description"),
      ).toBeInTheDocument();
    ...
    it("複数のリポジトリを表示する", () => {
      const repositories = [
        mockRepository,
        {
          ...
          id: 2,
          name: "another-repo",
          owner: { ...mockRepository.owner, login: "another-owner" },
        },
      ];
  ...
  describe("Interactions", () => {
    it("リポジトリクリック時にハンドラーが呼ばれる", () => {
      ...
      expect(onRepositoryClick).toHaveBeenCalledWith(
        mockRepository,
        expect.any(Object),
      );
    });
  ...
  describe("Live search integration", () => {
    it("ライブサーチフックが正しいクエリで呼ばれる", () => {
      render(<SearchResults {...defaultProps} searchQuery="react hooks" />);

      expect(mockUseLiveSearchFn).toHaveBeenCalledWith("react hooks");
    });
    ...
    it("ライブサーチの結果数が表示される", () => {
      ...
      expect(
        screen.getByText("「react」の検索結果: 1,234件"),
      ).toBeInTheDocument();
    });
  });
